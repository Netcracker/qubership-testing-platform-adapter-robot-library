...
public class BasicFormatTestSuiteReader implements TestSuiteReader {
    public static final String TEST_SUITE_BASIC_FOLDER = Config.getString("BasicFormatTestSuiteReader.TEST_SUITE_BASIC_FOLDER", null);
    ...
    public static final String FILE_SEPARATOR = Config.getString("BasicFormatTestSuiteReader.FILE_SEPARATOR",
    ...
    public BasicFormatTestSuiteReader(ExcelSheet excelSheet) {
        ...
        if (this.sourceIsValid()) {
            for(int i = 2; i <= this.getExcelSheet().getRowList().size(); ++i) {
                ...
                if (StringUtils.isBlank(tcName) && StringUtils.isBlank(testCaseValidationLevel) && StringUtils.isBlank(tcFilename) && StringUtils.isBlank(description) && StringUtils.isBlank(threadName) && StringUtils.isBlank(sectionName)) {
                    log.debug("Row '{}' was skipped because it is empty in file '{}'", i,
                            this.getExcelSheet().getExcelBook().getCurrentFile().getName());
                } else {
    ...
    protected final int parseValidationLevel(int rowNum, String testCaseValidationLevel) throws InvalidFormatOfSourceException {
        try {
            return ValidationLevel.parseValidationLevel(testCaseValidationLevel);
        } catch (IllegalArgumentException e) {
            throw new InvalidFormatOfSourceException(e, "Run column '%s' value '%s' is wrong in row '%s'. Error: %s", RUN_COLUMN_NAME, testCaseValidationLevel, rowNum, e.getMessage());
        }
    }

    protected boolean shouldBeExecuted(FileTestCase testCase) {
        if (this.filter.match(testCase)) {
            log.debug("Test case '{}' will be executed", testCase);
            return true;
        } else {
            log.debug("Test case '{}' is not specified for execution", testCase);
            return false;
        }
    }
