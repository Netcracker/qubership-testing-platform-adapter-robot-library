...
if [[ -n ${GITREPO} ]]; then
  ...
  do
    ...
    if ! git clone -b "${BRANCH}" https://"${USER}":"${PASS}""${REPO}" prj/ ; then
...
while getopts u:p:t:m:e:l: option
do
  case "${option}"
    in
    u) echo atp.ram.url="${OPTARG}" >> test.properties;;
    ...
    l) echo atp.logger.url="${OPTARG}" >> test.properties;;
    *) echo 'Invalid option'
  esac
done
